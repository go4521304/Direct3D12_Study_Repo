...
void Camera::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* commandList)
{
}
...
void Camera::UpdateShaderVariables(ID3D12GraphicsCommandList* commandList)
{
	...
	//루트 파라메터 인덱스 1의
	commandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4View, 0);
	XMFLOAT4X4 xmf4x4Projection;
	XMStoreFloat4x4(&xmf4x4Projection,
		XMMatrixTranspose(XMLoadFloat4x4(&m_xmf4x4Projection)));
	commandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4Projection, 16);
}
...
{
	commandList->RSSetViewports(1, &m_d3dViewport);
	commandList->RSSetScissorRects(1, &m_d3dScissorRect);
}
